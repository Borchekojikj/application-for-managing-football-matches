                    ...
                    <form action="{{ route('matches.update', ['id' => $match->id]) }}" method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" aria-label="Default select example" name="team_1_id">
                                ...
                                <option value="{{ $team->id }}" {{ $team->id == $match->team_1_id ? 'selected' : '' }}>{{ $team->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('team_1_id')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" aria-label="Default select example" name="team_2_id">
                                <option selected value="">Open this select menu</option>
                                @foreach($teams as $team)
                                <option value="{{ $team->id }}" {{ $team->id == $match->team_2_id ? 'selected' : '' }}>{{ $team->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('team_2_id')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" id="date_match" name="date_match" value="{{ old('date_match', $match->date_match) }}">
                        </div>
                        @error('date_match')
                        <p class="alert alert-danger"> {{ $message }}</p>
                        @enderror
